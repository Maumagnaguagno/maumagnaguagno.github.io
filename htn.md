...
The second is the ``recursion``, as the same suggests it will expand one ``move`` and recursively expands ``forward``, taking care to mark positions visited and later freeing such positions to be reused.
The ``travel`` method we suggested in the introduction could be implemented like this.
